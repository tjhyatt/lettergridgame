      ...
      >
        <div class="flex flex-col">
              ...
              <div class="flex flex-col items-center">
                ...
                <div v-if="isGameOver" class="relative mt-1">
                  <transition name="fade">
                    <div
                      v-if="isScoreCopied"
                      class="absolute -top-8 left-1/2 transform -translate-x-1/2 bg-gray-500 bg-opacity-90 text-white text-xs uppercase py-1 w-20 text-center rounded-md"
                    >
                      Score copied
                    </div>
                  </transition>
            ...
            <div class="flex flex-col mx-1">
              <div class="flex flex-nowrap items-end mb-2">
                <div class="w-24 text-sm uppercase flex-shrink-0">
                  Gold
                  <span v-if="awards[3]" class="text-xs"
                    >({{ awards[3] }})</span
                  >
                </div>
                <div v-if="awards[3]" class="w-full">
                  <div
                    class="bg-gold h-4 rounded-sm"
                    :style="`width: ${(awards[3] / awardsHighest) * 100}%`"
                  ></div>
                </div>
              </div>
              ...
              <div class="flex flex-nowrap items-end mb-2">
                <div class="w-24 text-sm uppercase flex-shrink-0">
                  Silver
                  <span v-if="awards[2]" class="text-xs"
                    >({{ awards[2] }})</span
                  >
                </div>
                <div v-if="awards[2]" class="w-full">
                  <div
                    class="bg-silver h-4 rounded-sm"
                    :style="`width: ${(awards[2] / awardsHighest) * 100}%`"
                  ></div>
                </div>
              </div>
              ...
              <div class="flex flex-nowrap items-end mb-2">
                <div class="w-24 text-sm uppercase flex-shrink-0">
                  Bronze
                  <span v-if="awards[1]" class="text-xs"
                    >({{ awards[1] }})</span
                  >
                </div>
                <div v-if="awards[1]" class="w-full">
                  <div
                    class="bg-bronze h-4 rounded-sm"
                    :style="`width: ${(awards[1] / awardsHighest) * 100}%`"
                  ></div>
                </div>
              </div>
              ...
              <div class="flex flex-nowrap items-end mb-2">
                <div class="w-24 text-sm uppercase flex-shrink-0">
                  No Award
                  <span v-if="awards[0]" class="text-xs"
                    >({{ awards[0] }})</span
                  >
                </div>
                <div v-if="awards[0]" class="w-full">
                  <div
                    class="bg-black h-4 rounded-sm"
                    :style="`width: ${(awards[0] / awardsHighest) * 100}%`"
                  ></div>
                </div>
              </div>
            </div>
          ...
          <div class="w-full h-64 mb-8">
            <div class="uppercase text-center mb-4">
              Game History <span class="text-xs opacity-70">last 7</span>
            </div>
...
export default {
  ...
  computed: {
    ...
    isActive() {
      return this.activeModal === "stat_modal";
    },
  ...
  mounted() {
    ...
    this.timer = setInterval(() => {
      ...
      this.timeToMidnight = Duration.fromObject({ hours: diff.hours }).toFormat(
        "hh:mm:ss"
      );
