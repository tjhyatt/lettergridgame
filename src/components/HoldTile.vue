...
export default {
  ...
  methods: {
    resetTile() {
      ...
      this.$emit("highlightHoldLetterReset");
    },

    clickStart(e) {
      this.isClicked = true;
      this.$refs.holdTile.setPointerCapture(e.pointerId);
    ...
    clickMove(e) {
      if (this.isClicked) {
        ...
        if (hoveredTile) {
          ...
          } else if (this.hoveredTileType === "hold") {
            const row = hoveredTile.getAttribute("row");

            this.hoveredTileIndices.row = row;
            this.hoveredTileIndices.col = null;
            this.$emit("highlightHoldLetter", row);
          } else {
            this.resetTile();
          }
        } else {
    ...
    clickEnd(e) {
      ...
      this.$refs.holdTile.releasePointerCapture(e.pointerId);
      ...
      if (this.hoveredTileType === "hold") {
        this.$emit("setHoldTile", this.tile, this.hoveredTileIndices);
        this.pos = {
          x: 0,
          y: 0,
        };
