            ...
            :class="[
              {
                ...
                'tile--highlight' : highlightedBoardLetters[rowIndex][colIndex] === 1,
                'tile--bonus' : isBonusTile(rowIndex, colIndex)
              },
              isBonusTile(rowIndex, colIndex) ? `tile--bonus-${isBonusTile(rowIndex, colIndex)}` : null
            ]"
          >
            <div type="board" :row="rowIndex" :col="colIndex" class="tile-slot tile__letter">{{ slot.letter }}</div>
...
export default {
  ...
  components: { NextTile, StatisticsModal, WordScore },
  ...
  methods: {
      ...
      for (let rowIndex = 0; rowIndex < rows; rowIndex++) {
        ...
        for (let columnIndex = 0; columnIndex < columnsToCheck; columnIndex++) {
          ...
          for (let wordCount = 0; wordCount < possibleWords; wordCount++) {
            ...
            // build the word
            for (let letter = columnIndex; letter < wordLength + columnIndex; letter++) {
              // check if letter is on a bonus tile
              let letterBonus = ''

              this.bonusTiles.forEach(bonus => {
                if (bonus.pos.length && bonus.pos[0] === rowIndex && bonus.pos[1] === letter) {
            ...
            let validWord = _.find(wordlist.words, checkWord => {
              return checkWord === possibleWord.toUpperCase() && possibleWord.length <= maxWordLength
            })
      ...
      for (let columnIndex = 0; columnIndex < columns; columnIndex++) {
        ...
        for (let rowIndex = 0; rowIndex < rowsToCheck; rowIndex++) {
          ...
          for (let wordCount = 0; wordCount < possibleWords; wordCount++) {
            ...
            // build the word
            for (let letter = rowIndex; letter < wordLength + rowIndex; letter++) {
              // check if letter is on a bonus tile
              let letterBonus = ''

              this.bonusTiles.forEach(bonus => {
                if (bonus.pos.length && bonus.pos[0] === letter && bonus.pos[1] === columnIndex) {
                  if (bonus.type === 'dl' || bonus.type === 'tl') {
                    letterBonus = bonus.type
                  }
    ...
    async setBoardTile (tile, hoveredIndices) {
      ...
      if (tile.letter.includes('|')) {
        ...
        let str = prompt(`Enter a ${letterOptions[0]} or ${letterOptions[1]}`, '')
      ...
      for (let columnIndex = 0; columnIndex < columnsToCheck; columnIndex++) {
        ...
        for (let wordCount = 0; wordCount < possibleWords; wordCount++) {
          ...
          for (let letter = columnIndex; letter < wordLength + columnIndex; letter++) {
            ...
            this.bonusTiles.forEach(bonus => {
              if (bonus.pos.length && bonus.pos[0] === tileRowIndex && bonus.pos[1] === letter) {
                if (bonus.type === 'dl' || bonus.type === 'tl') {
                  letterBonus = bonus.type
                }
      ...
      for (let rowIndex = 0; rowIndex < rowsToCheck; rowIndex++) {
        ...
        for (let wordCount = 0; wordCount < possibleWords; wordCount++) {
          ...
          for (let letter = rowIndex; letter < wordLength + rowIndex; letter++) {
            ...
            this.bonusTiles.forEach(bonus => {
              if (bonus.pos.length && bonus.pos[0] === letter && bonus.pos[1] === tileColIndex) {
                if (bonus.type === 'dl' || bonus.type === 'tl') {
                  letterBonus = bonus.type
                }
...
.board {
  ...
  .tile--bonus::before {
    align-items: center;
    border-radius: 50%;
    box-shadow: rgba(#000, .6) 0.1em 0.15em 0.15em;
    color: #fff;
    display: flex;
    font-size: .75em;
    justify-content: center;
